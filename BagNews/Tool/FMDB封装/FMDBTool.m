//
//  FMDBTool.m
//  dhsajkdhjkadhkasjdhasjkd
//
//  Created by dllo on 16/4/8.
//  Copyright © 2016年 WeiLiang. All rights reserved.
//
...

{
    ...
    NSArray *library = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    
    NSString *dbPath = [library[0] stringByAppendingPathComponent:[dbName stringByAppendingString:@".sqlite"]];
...
- (void)deleteData:(FMDatabase *)db from:(NSString *)tableName Dic:(NSDictionary *)dic
{
    if ([self isOpenDatabese:db]) {
        ...
        [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM %@ WHERE title = %@",tableName,str]];
