...
@interface JWAVPlayerViewController () <CFDanmakuDelegate>
@property (nonatomic, weak) CFDanumukuView* danmakuView;
...
- (void)setupDanmakuView
{
    ...
    CFDanumukuView* danmakuView = [[CFDanumukuView alloc] initWithFrame:rect];
...
- (void)getTheDanModle
{
    ...
    [webD loadRequest:reqD progress:&progress success:^NSString* _Nonnull(NSHTTPURLResponse* _Nonnull response, NSString* _Nonnull HTML) {
        ...
        if (HTML != nil) {
            ...
            NSDictionary* dict = [JWXML2Dic dicWithData:HTML];
...
- (void)setupDanmakuData:(NSDictionary*)dict
{
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj, BOOL* _Nonnull stop) {
        for (JWDanmuModel* danmu in obj) {
            
            CFDanmuku* danmaku = [[CFDanmuku alloc] init];
            NSMutableAttributedString* contentStr = [[NSMutableAttributedString alloc] initWithString:danmu.text.length > 0 ? danmu.text : @"2333" attributes:@{ NSFontAttributeName : [UIFont boldSystemFontOfSize:danmu.fontSize ? [danmu.fontSize floatValue] : 14.0],
                                                                                                                                                                 NSForegroundColorAttributeName : UIColorFromRGB(danmu.textColor ? ((long)[danmu.textColor doubleValue]) : 0xfff) }];
